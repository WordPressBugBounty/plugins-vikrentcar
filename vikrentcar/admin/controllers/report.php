...
/**
 * VikRentCar report controller.
 *
 * @since   1.15.9 (J) - 1.4.6 (WP)
 */
class VikRentCarControllerReport extends JControllerAdmin
{
    /**
    ...
    public function renderSettings()
    {
        ...
        $report_obj = VikRentCar::getReportInstance($report);

        if (!$report_obj) {
            // invalid report requested
            VRCHttpDocument::getInstance($app)->close(404, sprintf('Could not find the report [%s] to load the settings from.', $report));
        }
        ...
        VRCHttpDocument::getInstance($app)->json([
            'html' => $form_html,
        ]);
    }
    ...
    public function saveSettings()
    {
        ...
        if (!$report_obj) {
            // invalid report requested
            VRCHttpDocument::getInstance($app)->close(404, sprintf('Could not find the report [%s] to save the settings for.', $report));
        }
        ...
        if ($report_obj->allowsProfileSettings() && !empty($profile)) {
            if ($profile == '_new' && empty($profile_name)) {
                VRCHttpDocument::getInstance($app)->close(500, 'Please specify the name for the new settings profile.');
            }
    ...
    public function executeCustomAction()
    {
        ...
        if (!$report_obj) {
            // invalid report requested
            VRCHttpDocument::getInstance($app)->close(404, sprintf('Could not find the report [%s] for executing the action.', $report));
        }
        ...
        if (!in_array($action, array_column($actions, 'id'))) {
            // unsupported action
            VRCHttpDocument::getInstance($app)->close(403, sprintf('Unsupported report action [%s].', $action));
        }

        try {
            $result = $report_obj->executeAction($action, $scope, $data);
        } catch (Exception $e) {
            VRCHttpDocument::getInstance($app)->close($e->getCode(), $e->getMessage());
        }

        // send the response to output
        VRCHttpDocument::getInstance($app)->json($result);
    }
    ...
    public function setActiveProfile()
    {
        ...
        if (!$report_obj) {
            // invalid report requested
            VRCHttpDocument::getInstance($app)->close(404, sprintf('Could not find the report [%s].', $report));
        }
