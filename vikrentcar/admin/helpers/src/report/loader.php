...
final class VRCReportLoader
{
	...
	{
		return VikRentCar::getReportInstance($report);
	}
	...
	public function getDrivers($type = '')
	{
		...
		foreach ($report_files as $k => $report_path) {
			...
			$classname = 'VikRentCarReport' . str_replace(' ', '', ucwords(str_replace('_', ' ', $report_file)));
	...
	public function getInvolvedCountries()
	{
		...
		$q->from($dbo->qn('#__vikrentcar_countries'));
	...
	{
		return VRC_ADMIN_PATH . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'report' . DIRECTORY_SEPARATOR;
	}
}
