 ...
 *
 * jQuery(target).vrcContextMenu(  'trigger', 'click');
 * jQuery(target).vrcContextMenu('placement',  'auto');
 */
(function($) {
	/**
	...
	var vikPopupMenuInit = function(root, options) {
		...
		options = $.extend($.vrcContextMenu.defaults, options);
		...
		$(document).on('keydown.contextmenu.vikrentcar', function(event) {
			// ignore the event with this namespace because it will end up
			// to catch also the plain keydown event
			if (event.namespace == 'contextmenu.vikrentcar') {
				return true;
			}
	...
	var vikPopupMenuShow = function(root, event) {
		...
		$(document).on('mousedown.contextmenu.vikrentcar', function(event) {
			// ignore the event with this namespace because it will end up
	...
	var vikPopupMenuHide = function(root) {
		...
		if ($('.vik-context-menu').length) {
			...
			// turn off proxy
			$(document).off('mousedown.contextmenu.vikrentcar');
	...
	var vikPopupMenuDestroy = function(root) {
		...
		$(document).off('keydown.contextmenu.vikrentcar');
	...
	$(document).on('mousedown', function() {
		...
		$(document).trigger('mousedown.contextmenu.vikrentcar');
	});
	...
	$(document).on('keydown', function() {
		...
		$(document).trigger('keydown.contextmenu.vikrentcar');
	});

	// register the jQuery callback
	$.fn.vrcContextMenu = function(method, data) {
		if (!method) {
			method = {};
		}
